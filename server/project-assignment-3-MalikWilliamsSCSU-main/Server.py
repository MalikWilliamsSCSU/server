...
class server:
    ...
    def receiveFile(self, connectionSocket, fileName):
        file_path = self.path + "/" + fileName
        print(f"Receiving file: {fileName}, saving to: {file_path}")
    
        try:
            with open(file_path, 'wb') as f:
                while True:
                    # Receive data in chunks (1024 bytes)
                    data = connectionSocket.recv(1024)
                    if not data:
                        break  # No more data, file transfer is complete
